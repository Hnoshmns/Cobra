...
if (Config.WORKTYPE == 'private') {
    ...
    cobra.addCommand({ pattern: 'cricket ?(.*)', fromMe: true, desc: FONT_DESC,  deleteCommand: false}, async (message, match) => {
...
if (Config.WORKTYPE == 'public') {
    ...
    cobra.addCommand({ pattern: 'cricket ?(.*)', fromMe: false, desc: FONT_DESC,  deleteCommand: false}, async (message, match) => {
        if (!match[1]) return await message.client.sendMessage(message.jid,FONT_NEED,MessageType.text)
