...
wikipedia data, sentiment aanalysis, Instagram workflow with neural cells.
...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Eva') && eva_functionality !== 'true') {        
        ...
        await axios.get('http://api.brainshop.ai/get?bid=158831&key=pUfTJQ5dGLACtaMk&uid=' + unique_ident + '&msg=' + uren).then(async (response) => {
            var fins = ''                           
            if (conf.LANG !== 'EN') {
    ...
    try {
        ...
        convertToWav(file)
            .on('end', async () => {
                ...
                await axios.get('http://api.brainshop.ai/get?bid=158831&key=pUfTJQ5dGLACtaMk&uid=' + unique_ident + '&msg=' + son).then(async (response) => {
                    var trmsg = ''
