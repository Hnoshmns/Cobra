    ...
    return new Promise(async (resolve, reject) => {
        ...
        await Axios({
            ...
            await Axios({
                ...
                resolve({
                    status: true,
                    message: "Copyright © 2021 | 🐍COBRABOT🐍",
                    result: result
                })
            }).catch(reject)
        }).catch(reject)
        ...
        await webp2mp4File(savedFilename).then(async (rest) => {
            await Axios({ method: "GET", url: rest.result, responseType: "stream"}).then(({ data }) => {
                ...
                saving.on("finish", async () => {
                    await message.client.sendMessage(mid, fs.readFileSync('/root/WhatsAsenaDuplicated/stweb.mp4'), MessageType.video, { mimetype: Mimetype.mp4, caption: 'Copyright © 2021 | 🐍COBRABOT🐍', quoted: message.data })
